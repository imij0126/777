...
<body>
                    ...
                    <td colspan="3">
                        <c:choose>
                        	...
                        	<c:when test="${ not empty board.originName }">
                        		<a href="<%= request.getContextPath() %>/${ board.changeName }" download="${ board.originName }">${ board.originName }</a>
                        	</c:when>
	...
	<script>
		...
		function addReply(){
			...
			// 댓글 추가 요청(ajax) 
			//  => /spring/board/rinsert?replyContent=입력된내용&refBno=게시글번호&replyWrite=작성자
			$.ajax({
				url: 'rinsert',		//요청 주소
				data: {
				...
				success: function(result) {		// 요청 성공 시 (통신 성공)
					console.log(result); 
				// 댓글 추가 성공 시 , 입력창 부분 초기화 댓글 목록 다시 조회
					if(result === 'success'){
						$("#replyArea #content").val('');
						selectReplyList();
					}else{
				// 댓글 추가 실패 시 , '댓글 추가에 실패했습니다.' 메시지로 출력		
						alert('댓글 추가에 실패했습니다.');
					}
				},
				error: function(err){
